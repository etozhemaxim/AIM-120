...
class Geometry:
    ...
    x_Ak: float  = 0.0    #НАДО ПОМЕНЯТЬ ЭТО НЕ МОЕ ЗНАЧЕНИЕ !!!!!!!!!
    b_Ak_r: float = 0.025 #НАДО ПОМЕНЯТЬ ЭТО НЕ МОЕ ЗНАЧЕНИЕ !!!!!!!!!
    b_Ak_kr : float= 0.1  #НАДО ПОМЕНЯТЬ ЭТО НЕ МОЕ ЗНАЧЕНИЕ !!!!!!!!!

    x_b_kr: float = 1.26 # координата начала бортовой хорды крыла (от носа) НАДО ПОМЕНЯТЬ ЭТО НЕ МОЕ ЗНАЧЕНИЕ !!!!!!!!!
    x_b_rl: float = 2 # координата начала бортовой хорды руля (от носа) НАДО ПОМЕНЯТЬ ЭТО НЕ МОЕ ЗНАЧЕНИЕ !!!!!!!!!
    x_t = 1.914 
...
class FlowSetup:
    ...
    def default() -> FlowSetup:
        ...
        big_angles_deg = np.array([-25, -20, -15, -10, 10, 15, 20, 25], dtype=float)
...
class AerodynamicsModel:
    def __init__(self, geom: Geometry, aero: AerodynamicParams, grid: FlowSetup,
                 resistance: Optional['Resistance']=None,
                 i_is_wing: bool = True,           # нумерация I/II
                 front_is_wing: bool = True):       # физика front/back
        self.g = geom
        self.a = aero
        self.grid = grid
        self.i_is_wing = i_is_wing
        self.front_is_wing = front_is_wing
    ...
    # --- Мапперы поясов: кто считается I и II (только подписи) ---
    def deltas_I(self) -> np.ndarray:
        return self.grid.deltas_I_deg if self.i_is_wing else self.grid.deltas_II_deg

    def deltas_II(self) -> np.ndarray:
        return self.grid.deltas_II_deg if self.i_is_wing else self.grid.deltas_I_deg

    def as_I(self, kr_arr: np.ndarray, rl_arr: np.ndarray) -> np.ndarray:
        return kr_arr if self.i_is_wing else rl_arr

    def as_II(self, kr_arr: np.ndarray, rl_arr: np.ndarray) -> np.ndarray:
        return rl_arr if self.i_is_wing else kr_arr

    # --- Мапперы front/back (физика): выбирают, кто фронт/бэк в реальности ---
    def as_Front(self, kr_arr: np.ndarray, rl_arr: np.ndarray) -> np.ndarray:
        return kr_arr if self.front_is_wing else rl_arr

    def as_Back(self, kr_arr: np.ndarray, rl_arr: np.ndarray) -> np.ndarray:
        return rl_arr if self.front_is_wing else kr_arr

    # Площадные доли для I/II (для сумм в малых и больших углах)
    def S_I_bar(self) -> float:
        # площадь пояса I (по нумерации I/II)
        return self.g.S_1_bar if self.i_is_wing else self.g.S_2_bar

    def S_II_bar(self) -> float:
        return self.g.S_2_bar if self.i_is_wing else self.g.S_1_bar

    # Площадные доли для Front/Back (физика)
    def S_Front_bar(self) -> float:
        return self.g.S_1_bar if self.front_is_wing else self.g.S_2_bar

    def S_Back_bar(self) -> float:
        return self.g.S_2_bar if self.front_is_wing else self.g.S_1_bar
    ...
    def z_bar(self) -> np.ndarray:
        # bar_z_v зависит от геометрии передней поверхности
        lam_f = self.as_Front(self.a.lambda_kr, self.a.lambda_rl)
        chi_f = self.as_Front(self.a.chi_05_kr, self.a.chi_05_rl)
        zeta_f= self.as_Front(self.a.zeta_kr,  self.a.zeta_rl)
        fn = lambda M: AeroBDSMWrapper.bar_z_v(M, lam_f, chi_f, zeta_f)
        return AeroBDSMWrapper.vec(fn, self.grid.M)

    def z_v_meters(self) -> np.ndarray:
        zbar = self.z_bar()
        D = self.g.D
        l_span_front = self.as_Front(self.g.l_raszmah_kr, self.g.l_raszmah_rl)
        return (D + zbar * (l_span_front - D)) / 2.0

    def i_v(self, z_v: Optional[np.ndarray] = None) -> np.ndarray:
        # i_v зависит от геометрии ЗАДНЕЙ поверхности
        if z_v is None:
            z_v = self.z_v_meters()
        zeta_back = self.as_Back(self.a.zeta_kr, self.a.zeta_rl)
        l_span_back = self.as_Back(self.g.l_raszmah_kr, self.g.l_raszmah_rl)
        fn = lambda zv_: AeroBDSMWrapper.i_v(zeta_back, self.g.D, l_span_back, 1.0, zv_)
    ...
    def compute_eps_alpha_sr(self, K_aa_back: np.ndarray, k_aa_front: np.ndarray) -> np.ndarray:
        # front/back параметры
        z_v = self.z_v_meters()
        i_v = self.i_v(z_v)
        c_ya_front = self.as_Front(self.c_ya_is_kr(), self.c_ya_is_rl())
        lam_front  = self.as_Front(self.a.lambda_kr,     self.a.lambda_rl)
        l_front    = self.as_Front(self.g.l_raszmah_kr,  self.g.l_raszmah_rl)
        l_back     = self.as_Back( self.g.l_raszmah_kr,  self.g.l_raszmah_rl)
        ...
        eps = (57.3 / (2 * math.pi)) * (i_v / np.maximum(z_v, 1e-12)) \
              * (l_front / np.maximum(l_back, 1e-12)) \
              * (c_ya_front / np.maximum(lam_front, 1e-12)) \
              * (np.maximum(k_aa_front, 1e-12) / np.maximum(K_aa_back, 1e-12)) \
              * psi_eps
        return eps
                ...
                for phi_alpha in self.grid.phi_alpha_values:
                    for alpha_p in self.grid.phi_alpha_values:
                        for idx, M in enumerate(self.grid.M):
                            ...
                            eps = (57.3/(2*math.pi)) * (i_v_val/max(z_v,1e-12)) \
    ...
    def compute_small_angles(self) -> Tuple[np.ndarray, np.ndarray]:
        """
        Суммарная производная c_y^alpha(M) и значения c_y(M, alpha) для малых углов.
        Фронт/бэк берутся из флага front_is_wing:
        - front = крыло (по умолчанию),
        - или front = рули (если front_is_wing=False).
        """
        intr = self.compute_interference()

        # k/K для front/back
        K_aa_front = self.as_Front(intr['K_aa_kr'], intr['K_aa_rl'])
        K_aa_back  = self.as_Back( intr['K_aa_kr'], intr['K_aa_rl'])

        # Производные изолированных поверхностей
        cya_kr = self.c_ya_is_kr()
        cya_rl = self.c_ya_is_rl()

        cya_front = self.as_Front(cya_kr, cya_rl)
        cya_back  = self.as_Back( cya_kr, cya_rl)

        # Площадные доли front/back
        S_front_bar = self.S_Front_bar()
        S_back_bar  = self.S_Back_bar()

        # Торможение потока (как у вас): нос для front-пояса, IsP для back-пояса
        ...
        # Скос от фронта к бэку
        # eps_alpha_sr рассчитывается с учётом front/back и (k_aa_front / K_aa_back)
        eps_alpha_sr = self.compute_eps_alpha_sr(K_aa_back=K_aa_back,
                                                k_aa_front=self.as_Front(intr['k_aa_kr'], intr['k_aa_rl']))

        # Суммарная производная c_y^alpha:
        # front-вклад умножаем на (1 - eps/57.3), back-вклад без множителя
        cya_sum = (cya_f * self.g.S_f_bar
                + cya_front * K_aa_front * (1.0 - eps_alpha_sr / 57.3) * S_front_bar * kq_nos
                + cya_back  * K_aa_back                              * S_back_bar  * kq_isp)

        # Линейный закон для малых углов: c_y = cya_sum * alpha
    ...
    def compute_eps_delta_sr(self, K_aa_back: np.ndarray, k_delta_0_front: np.ndarray) -> np.ndarray:
        M = self.grid.M
        ...
        eps = (57.3/(2*math.pi)) * (i_v / np.maximum(z_v,1e-12)) \
              * (l_front / np.maximum(l_back, 1e-12)) \
              * (c_ya_front / np.maximum(lam_front, 1e-12)) \
              * ((k_delta_0_front * n_vals) / np.maximum(K_aa_back, 1e-12)) \
              * psi_eps
        return eps
    ...
    def compute_cy_delta_1_2(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        k_d0_kr, K_d0_kr, k_d0_rl, K_d0_rl = self.compute_delta_derivatives()
        k_d0_front = self.as_Front(k_d0_kr, k_d0_rl)
        k_d0_back  = self.as_Back( k_d0_kr, k_d0_rl)
        K_d0_front = self.as_Front(K_d0_kr, K_d0_rl)
        K_d0_back  = self.as_Back( K_d0_kr, K_d0_rl)
        ...
        cya_kr = self.c_ya_is_kr()
        cya_rl = self.c_ya_is_rl()
        cya_I  = self.as_I(cya_kr, cya_rl)
        cya_II = self.as_II(cya_kr, cya_rl)

        kq_nos = self.kappa_q_nos()
        kq_isp = self.kappa_q_isp()

        # eps_delta_sr считаем как в вашей компоновке (от крыла к рулю),
        # если у товарища только другая нумерация — физика не меняется
        eps_delta_sr = self.compute_eps_delta_sr(K_aa_back=K_aa_back, k_delta_0_front=k_d0_front)

        cya_I  = self.as_I(self.c_ya_is_kr(), self.c_ya_is_rl())
        cya_II = self.as_II(self.c_ya_is_kr(), self.c_ya_is_rl())
        c_y_delta_1 = (cya_I  * K_d0_front * self._n_eff(self.grid.M) * self.S_I_bar() * self.kappa_q_nos()) \
                      - (cya_II * K_aa_back  * self.S_II_bar() * self.kappa_q_isp()) * eps_delta_sr

        c_y_delta_2 = (cya_II * K_d0_back  * self._n_eff(self.grid.M) * self.S_II_bar() * self.kappa_q_isp())
    ...
    def c_y_f_big(self) -> np.ndarray:
        ...
        term1 = 57.3 * cya_f * kappa * np.sin(ang_rad) * np.cos(ang_rad)
    ...
    def c_n_from_alpha_eff(self, alpha_eff: np.ndarray, cya_is: np.ndarray, A_is: np.ndarray) -> np.ndarray:
        ...
        return 57.3 * cya_is[:, None, None] * sin_ae * np.cos(alpha_eff) + A_is[:, None, None] * (sin_ae**2) * np.sign(alpha_eff)
    ...
    def compute_big_angles(self) -> Dict[str, np.ndarray]:
        ...
        intr = self.compute_interference()
        # k/K для крыла и рулей
        k_aa_kr, K_aa_kr = intr['k_aa_kr'], intr['K_aa_kr']
        k_aa_rl, K_aa_rl = intr['k_aa_rl'], intr['K_aa_rl']

        # Маппинг в пояса I/II
        k_aa_I  = self.as_I(k_aa_kr, k_aa_rl)
        K_aa_I  = self.as_I(K_aa_kr, K_aa_rl)
        k_aa_II = self.as_II(k_aa_kr, k_aa_rl)
        K_aa_II = self.as_II(K_aa_kr, K_aa_rl)

        # Δ для I и II с учетом соглашения
        dI = self.deltas_I()    # np.ndarray
        dII = self.deltas_II()

        c_y_f = self.c_y_f_big()

        # Изолированные производные → I/II
        cya_is_kr = self.c_ya_is_kr()
        cya_is_rl = self.c_ya_is_rl()
        cya_is_I  = self.as_I(cya_is_kr, cya_is_rl)
        cya_is_II = self.as_II(cya_is_kr, cya_is_rl)

        # A для I/II
        A_kr = self._A_is(cya_is_kr, for_wing=True)
        A_rl = self._A_is(cya_is_rl, for_wing=False)
        A_I  = self.as_I(A_kr, A_rl)
        A_II = self.as_II(A_kr, A_rl)

        n_vals = self._n_eff(self.grid.M)

        k_d0_kr, K_d0_kr, k_d0_rl, K_d0_rl = self.compute_delta_derivatives()
        k_d0_I  = self.as_I(k_d0_kr, k_d0_rl)
        k_d0_II = self.as_II(k_d0_kr, k_d0_rl)
        K_d0_I  = self.as_I(K_d0_kr, K_d0_rl)
        K_d0_II = self.as_II(K_d0_kr, K_d0_rl)

        # Эффект скоса (eps) — от переднего пояса (у нас крыло) к заднему (рули).
        # Если у товарища просто иная нумерация, физика не меняется — eps остаётся от крыла к рулю.
        z_v = self.z_v_meters()
        i_v = self.i_v(z_v)
        ...
        # alpha_eff[I] без eps для усреднения c_nI
        # Эффективные углы без eps для оценки среднего c_n по M
        alpha_eff_I0 = self.alpha_eff(angles, dI, k_aa_I, np.zeros_like(k_aa_I), n_vals, eps=None, sqrt2=True)  # (M, A, DI)
        c_nI0 = self.c_n_from_alpha_eff(alpha_eff_I0, cya_is_I, A_I)  # (M, A, DI)

        # Усредняем по углам и по оси delta_I → (M,)
        c_nI_mean_M = np.mean(c_nI0, axis=(1, 2))

        # eps_delta_sr: front → back (как у вас настроено)
        eps_delta_sr = (57.3/(2*math.pi)) * (i_v / np.maximum(z_v, 1e-12)) \
                    * (self.g.l_raszmah_kr / self.g.l_raszmah_rl) \
                    * (c_nI_mean_M / np.maximum(self.a.lambda_kr, 1e-12)) \
                    * ((k_d0_kr * n_vals) / np.maximum(K_aa_rl, 1e-12)) \
                    * psi_eps_zeros


        # Эффективные углы
        alpha_eff_I  = self.alpha_eff(angles, dI,  k_aa_I,  k_d0_I,  n_vals, eps=None,          sqrt2=True)
        alpha_eff_II = self.alpha_eff(angles, dII, k_aa_II, k_d0_II, n_vals, eps=eps_delta_sr,  sqrt2=True)

        # Подъёмная сила по поясам
        c_y_I, c_y_II = self.c_y_I_II_big(alpha_eff_I, alpha_eff_II,
                                          cya_is_I, cya_is_II, A_I, A_II,
                                          K_aa_I, k_aa_I, K_aa_II, k_aa_II)

        # Сумма
        c_y_sum_big = (c_y_f[:, :, None, None] * self.g.S_f_bar
               + c_y_I[:, :, :, None]  * self.S_I_bar()  * k_aa_I[:, None, None, None]
               + c_y_II[:, :, None, :] * self.S_II_bar() * k_aa_II[:, None, None, None])


        return dict(
            c_y_f=c_y_f,
            A_kr=A_kr, A_rl=A_rl,  # оставим для отладки
...
# финальная поебаотааааааааа 4.2 вроде
...
class Moments:
    ...
    def _as_I(self, kr_arr: np.ndarray, rl_arr: np.ndarray) -> np.ndarray:
        return kr_arr if self.model.i_is_wing else rl_arr

    def _as_II(self, kr_arr: np.ndarray, rl_arr: np.ndarray) -> np.ndarray:
        return rl_arr if self.model.i_is_wing else kr_arr
...
# ------------------------4.1  Фокус корпуса------------------------
    ...
    def x_Fi_f_I_II (self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        # Безопасные L_bar (на дозвуке = 0), b_bar — просто b/D
        ...
        # Базовые безразмерные координаты x_F_b_bar
        # ВАЖНО: векторизованные вызовы
        ...
        # Перестановка на выходе по флагу (чтобы I/II совпадали со схемой)
        xF_I  = self._as_I(x_Fi_f_I,  x_Fi_f_II)
        xF_II = self._as_II(x_Fi_f_I, x_Fi_f_II)
        return xF_I, xF_II
  # (M,), (M,)
    ...
    def x_F_a_I_II(self ,kaa_kr: np.ndarray,Kaa_kr: np.ndarray,kaa_rl: np.ndarray,Kaa_rl: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        # Внимание: kaa/Kaa здесь должны быть (M,)
        ...
        x_F_a_II = (x_F_is_kr + (kaa_rl - 1.0)*x_F_delta_rl + (Kaa_rl - kaa_rl)*x_Fi_f_II) / np.maximum(Kaa_rl, 1e-12)
                # Возврат в смысле I/II
        xF_I  = self._as_I(x_F_a_I,  x_F_a_II)
        xF_II = self._as_II(x_F_a_I, x_F_a_II)
        return xF_I, xF_II
...
# 5.56
    def x_F_delta_I_II(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        k_d0_back, K_d0_back, k_d0_front, K_d0_front = self.model.compute_delta_derivatives()
        intr  = self.model.compute_interference()
        ...
        x_F_delta1, _ = self.x_F_a_I_II(k_d0_back, K_d0_back, k_d0_front, K_d0_front)
        ...
        eps = self.model.compute_eps_delta_sr(K_aa_rl=Kaa_rl, k_delta_0_kr=k_d0_back)  # (M,)
    ...
    M, self.aero.lambda_kr, self.aero.chi_05_kr, self.aero.zeta_kr)  # (M,)
        ...
        # 5.56 (ваша запись)
        x_F_delta_I = ( (c_ya_is_kr*K_d0_back*n_vals*S_bar_I*k_I)*x_F_delta1
                        - (c_ya_is_kr*Kaa_rl*S_bar_II*k_II)*eps*x_F_a_II ) / np.maximum(c_y_delta_1, 1e-12)

        # 5.58
        x_F_delta_II = ( k_d0_front*x_F_is_kr + (K_d0_front - k_d0_front)*x_Fi_f_II ) / np.maximum(K_d0_front, 1e-12)

                # Возврат в смысле I/II
        xF_I  = self._as_I(x_F_delta_I,  x_F_delta_II)
        xF_II = self._as_II(x_F_delta_I, x_F_delta_II)
        return xF_I, xF_II
    ...
    def m_z(self) -> np.ndarray:
        ...
        DI  = np.radians(self.model.deltas_I())[None, None, :, None]
        DII = np.radians(self.model.deltas_II())[None, None, None, :]
...
# ========= 6) Экспорт (опционально — запускать локально) =========
...
class DataExporter:
    ...
    def build_all(self) -> Dict[str, 'pd.DataFrame']:
        ...
        dfs = {
            ...
            "c_y_small_angles.csv": m.assemble_c_y_small_angles_df(),
            "c_y_sum_big_delta.csv": m.assemble_c_y_sum_big_delta_df(),
            "c_y_sum.csv": m.assemble_c_y_sum(),
