...
class Geometry:
    ...
    S_rl: float = 0.2125
    S_f: float = 0.0248
    L_xv_kr: float = 2  # Расстояние от середины бортовой хорды крыла до кормового среза 
    L_xv_rl: float = 0.300    # Расстояние от середины бортовой хорды руля до кормового среза 
    # Исходя из геометрии моего крыла и рис. 5.1 стр. 253
    x_Ak: float  = 0    
    b_Ak_r: float = 0.26 
    b_Ak_kr : float= 0.8 

    x_b_kr: float = 1.632 # координата начала бортовой хорды крыла (от носа) 
    x_b_rl: float = 3.632 # координата начала бортовой хорды руля (от носа) 
    x_t : float = -0.189
