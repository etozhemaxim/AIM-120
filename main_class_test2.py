...
class Geometry:
    ...
    b_b_kr: float = 0.1
    b_b_rl: float = 0.025
    l_raszmah_kr: float = 0.0722 + 0.036*2
    l_raszmah_rl: float = 0.0722+0.033*2
    S_kr: float = ((l_raszmah_kr - 0.0722) / 2 ) * b_b_kr  * 4
    S_rl: float = ((l_raszmah_rl - 0.0722) / 2)  * b_b_rl * 4
    S_f: float = math.pi * 0.0722**2 / 4
    L_xv_kr: float = 1.635-0.368  # Расстояние от середины бортовой хорды крыла до кормового среза НАДО ПОМЕНЯТЬ ЭТО НЕ МОЕ ЗНАЧЕНИЕ !!!!!!!!!
    L_xv_rl: float = 0.085   # Расстояние от середины бортовой хорды руля до кормового среза НАДО ПОМЕНЯТЬ ЭТО НЕ МОЕ ЗНАЧЕНИЕ !!!!!!!!!
    ...
    x_b_kr: float = 1.5 #0.073*5# координата начала бортовой хорды крыла (от носа) НАДО ПОМЕНЯТЬ ЭТО НЕ МОЕ ЗНАЧЕНИЕ !!!!!!!!!
    x_b_rl: float = 0.073*5 # координата начала бортовой хорды руля (от носа) НАДО ПОМЕНЯТЬ ЭТО НЕ МОЕ ЗНАЧЕНИЕ !!!!!!!!!1.5
    x_t : float = 0.6938
...
class AerodynamicsModel:
    ...
    def get_n_Y_rasp(self) -> np.ndarray:
        ...
        # итог: (M,A,DI,DII)
        c_y_sum_rasp = cya*alpha_bal + d1*DI + d2*delta_bal

        # стало
        q = 0.5 * self.atm.rho(h) * (M * self.c)**2  # (M,)
        Y_rasp = c_y_sum_rasp * q[:, None, None, None]  # (M,1,1,1) корректный broadcast
    ...
    def assemble_n_y_rasp_df(self, use_big_angles: bool = False) -> 'pd.DataFrame':
        ...
        # тензор располагаемой перегрузки: (M, A, DI, DII); у вас A=1
        n_y_rasp = self.get_n_Y_rasp()

        # балансировочные углы (для инфо-колонок)
        alpha_bal, delta_bal = self.moments.alpha_delta_bal()   # alpha_bal: (M,1,1,DII), delta_bal: (M,A,1,1)
        alpha_bal_deg = np.degrees(alpha_bal)                   # (M,1,1,DII)
        delta_bal_deg = np.degrees(delta_bal)                   # (M,A,1,1)

        M_vals   = self.grid.M
        A_vals   = self.grid.small_angles_deg
        DI_vals  = self.grid.deltas_I_deg
        DII_vals = self.grid.deltas_II_deg

        M_len, A_len, DI_len, DII_len = n_y_rasp.shape

        M_list, alpha_list, delta_I_list, delta_II_list = [], [], [], []
        n_y_list, alpha_bal_list, delta_bal_list = [], [], []

        for i in range(M_len):
            for ai in range(A_len):  # как правило A_len == 1
                alpha_deg_val = float(A_vals[ai if ai < len(A_vals) else -1])
                # delta_bal не зависит от dj, поэтому берём одно значение для (i, ai)
                delta_bal_val_deg = float(delta_bal_deg[i, ai, 0, 0])
                for di in range(DI_len):
                    for dj in range(DII_len):
                        M_list.append(float(M_vals[i]))
                        alpha_list.append(alpha_deg_val)               # из сетки (как раньше)
                        delta_I_list.append(float(DI_vals[di]))
                        delta_II_list.append(float(DII_vals[dj]))      # из сетки (как раньше)

                        n_y_list.append(float(n_y_rasp[i, ai, di, dj]))

                        # alpha_bal — это рассчитанный α для данного δ_II (берем по dj)
                        alpha_bal_list.append(float(alpha_bal_deg[i, 0, 0, dj]))
                        # delta_bal — рассчитанная дельта для данного alpha (по ai)
                        delta_bal_list.append(delta_bal_val_deg)

        df = pd.DataFrame({
            "Mach": M_list,
            "alpha_deg": alpha_list,        # из сетки small_angles_deg
            "delta_I_deg": delta_I_list,    # из сетки deltas_I_deg
            "delta_II_deg": delta_II_list,  # из сетки deltas_II_deg
            "n_y_rasp": n_y_list,
            "alpha_bal": alpha_bal_list,    # рассчитанный α(δ_II)
            "delta_bal": delta_bal_list     # рассчитанный δ(α)
        })

        df.attrs['angle_type'] = "small"
        return df
...
# 5.56
    ...
    def test(self) -> Tuple[np.ndarray, np.ndarray]:
        # Углы (для форм)
        A   = np.radians(self.grid.small_angles_deg)[None, :, None, None]  # (1,A,1,1)
        DI  = np.radians(self.grid.deltas_I_deg)[None, None, :, None]      # (1,1,DI,1)
        DII = np.radians(self.grid.deltas_II_deg)[None, None, None, :]     # (1,1,1,DII)

        # Производные
        cya_sum = self.model.compute_small_angles()[0][:, None, None, None]     # (M,1,1,1)
        cy_d2   = self.model.compute_cy_delta_1_2()[1][:, None, None, None]     # (M,1,1,1)

        # Фокусы
        x_Fa = self.x_Fa_f()                        # (M,1) или (M,1,1,1)
        x_F_delta_I, x_F_delta_II = self.x_F_delta_I_II()  # (M,), (M,)

        x_t = self.geom.x_t
        L   = self.geom.l_f

        # Приводим фокусы к (M,1,1,1)
        xFa_b  = x_Fa[:, None, None, :] if x_Fa.ndim == 2 else x_Fa[:, None, None, None]   # (M,1,1,1)
        xF2_b  = x_F_delta_II[:, None, None, None]                                          # (M,1,1,1)

        # Наклоны по углам (без умножения на сами углы!)
        S_alpha = cya_sum * ((x_t - xFa_b)/L)   # (M,1,1,1)
        S_delta = cy_d2   * ((x_t - xF2_b)/L)   # (M,1,1,1)

        return S_alpha, S_delta
    ...
    def alpha_delta_bal(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        alpha_bal = - ( m_z_deltaII / m_z_a ) * DII #из-за добавления DII
...
class DataExporter:
    ...
    def build_all(self) -> Dict[str, 'pd.DataFrame']:
        ...
        dfs = {
            ...
            "n_y_rasp_test.csv": m.assemble_n_y_rasp_df(),
