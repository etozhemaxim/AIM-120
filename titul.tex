\documentclass[Homework.tex]{subfiles}

\begin{document}

\begin{titlepage}
\newpage
% шапка
\begin{flushleft}
{\footnotesize
    \begin{tabularx}{\textwidth}{c m{0.93\textwidth}}
        \begin{minipage}{0.07\textwidth}
            \includegraphics[width=1.9\linewidth]{images/BMSTU}
        \end{minipage}
        &
        \begin{center}
            \bfseries
            Министерство науки и высшего образования Российской Федерации\\
            Федеральное государственное бюджетное образовательное учреждение\\
            высшего образования\\
            <<Московский государственный технический университет имени Н.Э.~Баумана\\
            (национальный исследовательский университет)>>\\
            (МГТУ им. Н.Э. Баумана)
        \end{center}
    \end{tabularx}
}
\end{flushleft}

\begin{center}
    \par\rule{\textwidth}{1mm}
    \begin{flushleft}
        \small{\MakeUppercase{Факультет} \tabto{3cm} <<Специальное машиностроение>> (СМ)}\\
        \vspace{5mm}
        \small{\MakeUppercase{Кафедра} \tabto{3cm} <<Ракетные и импульсные системы>> (СМ-6)}\\
    \end{flushleft}
    \vspace{3cm}
    {
    {\fontsize{26}{30}\selectfont {РАСЧЁТНО-ПОЯСНИТЕЛЬАЯ ЗАПИСКА }}\\
    \vspace{3mm}
    {\fontsize{26}{30}\selectfont {К КУРСОВОЙ РАБОТЕ}}\\
    \vspace{7mm}
    {\large по дисциплине: {\fontsize{16}{18}\selectfont{Проектирование ракетного оружия}}}\\
        \vspace{3mm}
        {\large на тему: <<Определение аеродинамических характеристик ЛА>>}\\
        \vspace{1.5cm}
        {\large Вариант №{228}}\\
    }
    \vspace{3cm}
    {\small
    \begin{tabularx}{\textwidth}{p{7cm}p{0.1cm}p{3cm}p{0.1cm}X}
        Выполнил: студент группы {СМ6-72}  &  & \phantom{(подпись, дата)} & & {М.В. Ерофеев} \\ \cline{3-3}
        & & \hfil\scriptsize \raisebox{5pt}{(подпись, дата)}\hfil & & \\
        & & & & \\
        & & & & \\
        Проверил: &  & \phantom{(подпись, дата)} & & {Л.А. Лаптева} \\ \cline{3-3}
        & & \hfil\scriptsize \raisebox{5pt}{(подпись, дата)}\hfil & & \\
    \end{tabularx}
    }
    \vfill
    Москва, \the\year\ г.
\end{center}
\clearpage
\end{titlepage}
\setcounter{page}{2}

\end{document}